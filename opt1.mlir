// -----// IR Dump Before DecomposeUnsupportedAMDConversions (decompose-unsupported-amd-conversions) ('builtin.module' operation) //----- //
...
#shared = #triton_gpu.shared<{vec = 4, perPhase = 1, maxPhase = 4, order = [1, 0], hasLeadingOffset = false}>
#shared1 = #triton_gpu.shared<{vec = 64, perPhase = 1, maxPhase = 4, order = [0, 1], hasLeadingOffset = false}>
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 2 : i32, triton_gpu.target = "hip:gfx942", "triton_gpu.threads-per-warp" = 64 : i32} {
  ...
  ^bb2:  // pred: ^bb0
    ...
    %65:3 = scf.for %arg19 = %c0_i32 to %32 step %c1_i32 iter_args(%arg20 = %cst_5, %arg21 = %cst_8, %arg22 = %cst_9) -> (tensor<16x128xf32, #mma>, tensor<16xf32, #triton_gpu.slice<{dim = 1, parent = #mma}>>, tensor<16xf32, #triton_gpu.slice<{dim = 1, parent = #mma}>>)  : i32 {
      ...
      %113 = tt.trans %107 {order = array<i32: 1, 0>} : tensor<64x128xf16, #blocked> -> tensor<128x64xf16, #blocked2> loc(#loc54)
      %114 = triton_gpu.local_alloc %113 : (tensor<128x64xf16, #blocked2>) -> !tt.memdesc<128x64xf16, #shared1, #triton_gpu.shared_memory> loc(#loc54)
      %115 = triton_gpu.local_load %114 : !tt.memdesc<128x64xf16, #shared1, #triton_gpu.shared_memory> -> tensor<128x64xf16, #triton_gpu.dot_op<{opIdx = 1, parent = #mma, kWidth = 64}>> loc(#loc54)
      %116 = tt.dot %58, %115, %cst_1 : tensor<16x128xf16, #triton_gpu.dot_op<{opIdx = 0, parent = #mma, kWidth = 4}>> * tensor<128x64xf16, #triton_gpu.dot_op<{opIdx = 1, parent = #mma, kWidth = 64}>> -> tensor<16x64xf32, #mma> loc(#loc54)
      ...
      %135 = triton_gpu.local_load %134 : !tt.memdesc<64x128xf16, #shared2, #triton_gpu.shared_memory> -> tensor<64x128xf16, #triton_gpu.dot_op<{opIdx = 1, parent = #mma, kWidth = 64}>> loc(#loc51)
      %136 = tt.dot %133, %135, %131 : tensor<16x64xf16, #triton_gpu.dot_op<{opIdx = 0, parent = #mma, kWidth = 4}>> * tensor<64x128xf16, #triton_gpu.dot_op<{opIdx = 1, parent = #mma, kWidth = 64}>> -> tensor<16x128xf32, #mma> loc(#loc68)
